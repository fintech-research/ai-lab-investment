...
- Full-model option value in λ (fig-lambda-option-value, `lambda_option_value_curve`): *decreasing* for λ ≲ 0.025 (where φ*(λ) < φ̲), then increasing, convex up to λ ≈ 0.085 and CONCAVE beyond; over the policy range [0.1, 0.5] it is increasing and concave — the news-asymmetry prediction is that bad timeline news moves valuations more than good news
