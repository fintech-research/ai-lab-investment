...
\(\ell = 0.65\), with \(K_F\) growing in step. Meanwhile, the follower's
default boundary \(X_{D,F}\) rises roughly threefold over the same
range, as higher coupon payments increase the demand level at which
equity holders abandon the firm. The dashed line is the
\emph{follower's} default boundary \(X_{D,F}\), and the shaded band
between it and \(X_F^*\) is the follower's post-entry continuation band:
once installed, the follower operates there and above \(X_F^*\); before
entry it is waiting. The leader's boundary \(X_{D,L}\) (dash-dotted)
lies far below, because the leader enters at its much smaller
monopoly-phase scale. Although this region widens in absolute terms, the
follower's \emph{relative} distance to default contracts sharply:
\(X_F^*/X_{D,F}\) falls from approximately \(12\) at \(\ell = 0.05\) to
...
The leader's trigger \(X_P\) lies well below the follower's, reflecting
the preemption incentive. This compressed distance to default is the
scenario Amodei describes: a firm that commits to trillion-dollar
compute purchases faces bankruptcy if revenue falls even modestly short
of projections, because ``there's no hedge on Earth'' that can bridge
the gap \citep{amodei2026dwarkesh}.
...
logarithmic, so vertical distances are proportional distances to
default. The equilibrium is re-solved at each of 40 leverage levels
spanning \(\ell \in [0.05, 0.65]\): the follower's capacity and training
fraction are re-optimized at every level, while the leader's are held at
the monopoly-phase optimum under the leader-scale convention of
...
values. The leader's capacity and training fraction are held at the
monopoly-phase optimum (the leader-scale convention of
Section~\ref{sec-conventions}), on which the level of \(X_P\) depends:
re-optimizing the leader's scale for entry at \(X_P\) roughly doubles
the preemption discount (Internet Appendix B).}
...
five-year default probability under the true demand process (a
first-passage probability under the L-regime drift that omits the regime
switch, and therefore an upper bound on the pre-switch probability;
Internet Appendix H) reveals a complementary asymmetry, evaluated at
each firm's own optimal policy rather than at the fixed \(X = 0.10\),
\(K = 1\), and \(\phi = 0.5\) used for the spreads above: at
\(\ell = 0.40\), a conservative firm
...
a moderate adjustment cost and to the lower bound when re-purposing is
free. The exercise solves allocation and value only, not default or
belief mismatch, so what it says about the headline mechanisms is a
conjecture rather than a result: because the Proposition 2 solvency
floor and the Numerical Finding 1 asymmetry run through the \emph{level}
of pre-switch training investment, the lower \(\phi_1\) would attenuate
both, and the small value gain does not bound that attenuation, since
the observable \(\phi_1\) moves far more than the value does. One
consideration that exercise does not capture is the emergence of
\emph{inference-time scaling} (chain-of-thought reasoning,
...
eight times the baseline (both no-switch upper bounds on the pre-switch
probability). Both value-loss percentages are those of the reduced
model, computed under the pure-power discounting convention used to
price the option; re-solving the exact piecewise stopping problem
...
points over \((X_D, X_L^{\text{mono}})\), check that the gap changes
sign exactly once (from negative to positive), and refine the root with
Brent's method on the bracketing interval; the check is enforced on
every path that produces a reported number (the solver raises when it
fails), and it certifies a single crossing at the grid's resolution
rather than proving uniqueness. No case of multiple up-crossings was
...
risk-neutral (pricing-measure) object or a market credit spread.
