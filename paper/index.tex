...
to capacity, with the curvature parameter motivated by empirical scaling
laws \citep{kaplan2020scaling, hoffmann2022training}. The primary
...
the firm. The key difference is that their framework does not include
strategic interaction, training-inference allocation, or default risk.
The capacity-choice side of the problem is also the subject of a large
operations management literature, surveyed by
...
(Figure~\ref{fig-comparative-statics}). Two of the patterns are not
obvious. Cost convexity \(\gamma\) moves both the trigger and the
optimal capacity non-monotonically: on the plotted range \(K_H^*\) falls
from \(0.0107\) at \(\gamma = 1.2\) to a minimum near \(\gamma = 1.3\)
and rises to \(0.035\) at \(\gamma = 2\), because changing \(\gamma\) at
a fixed cost scale \(c\) raises marginal cost for capacities above the
normalized unit \(K = 1\) and lowers it below, and the optimum sits far
below that unit. A higher operating cost \(\delta\) raises \emph{both}
the trigger and the optimal capacity; that comovement is a rescaling of
the capacity unit rather than an economic sensitivity
...
wide margin at baseline (\(\Phi_L \approx 1.67\)), states that the
standalone L-regime scale problem has no interior optimum (the option
factor diverges as \(K \to 0\)), so that problem cannot pin \(A_1\)
down. It does not by itself imply \(A_1 = 0\). Internet Appendix A (Step
5b of the Proof of Proposition 1) shows that the replication argument
that suggests itself does not sign the coefficient: the candidate bound
...
coefficient \(A_{\text{eff}}\) that combines both regimes. Because
\(K^*\) and \(\phi^*\) are both in closed form (Proposition 1 and
Equation~\ref{eq-phi-star}), Equation~\ref{eq-trigger-phi} is explicit
once they are substituted: the trigger is the option premium times the
ratio of the total cost to the effective revenue coefficient, both
evaluated at the optimal policy (Internet Appendix A). The option
...
that the leader enters strictly earlier, is robust to the convention;
the magnitude is not, and neither is the monotone widening in
\(\sigma\): the re-optimized discount is \(94\%\) at \(\sigma = 0.20\),
\(86\%\) at \(0.25\), and \(87\%\) at \(0.30\) (Internet Appendix B).
For the leader, and by construction, competition in this solution
...
2026. Revenue covers AI-related activities; the CapEx row mixes cloud
compute commitments (Anthropic-like and OpenAI-like archetypes) with
total infrastructure spending (Google-like and xAI-like archetypes).
\(\hat{\phi}\) is the training fraction \emph{assigned} to each
archetype by judgment from three kinds of evidence (executive statements
...
training fraction \(\phi\). All values are order-of-magnitude
composites, not precise calibrations.}\label{tbl-firms}\tabularnewline
\toprule\noalign{}
...
the three archetypes modelled on privately held firms (Anthropic-like,
OpenAI-like, and xAI-like), the values rest on press reports and company
communications rather than audited filings and carry lower confidence
than the Google-like archetype's; they should be read as
order-of-magnitude inputs. Because the revenue and CapEx concepts are
not harmonized across the four archetypes (sector revenue versus segment
revenue, cloud commitments versus owned infrastructure), the columns of
...
The training fraction \(\hat{\phi}\) values represent my assignment of
the approximate share of compute allocated to model training rather than
to inference serving. They are inherently uncertain, because firms do
not publicly disclose precise training-inference breakdowns. I
triangulate the entries in Table~\ref{tbl-firms} from three types of
evidence: executive statements, the only firm-level compute
decomposition I could locate (Epoch AI's reconstruction of OpenAI's 2024
spend from investor documents shared with journalists and from press
reports, with an assumed amortization schedule), and industry-wide
trajectory estimates. Internet Appendix C sets out each in detail. The
xAI-like compute racer allocates the most to training
(\(\hat{\phi} = 0.75\)), consistent with a strategy of dedicating owned
compute (the Colossus supercluster) to frontier model training while
outsourcing inference to cloud providers.\footnote{\url{https://x.com/elonmusk/status/1947701807389515912}.}
...
more informative than any individual value. Because the \(\hat{\phi}\)
values are assigned rather than measured, the inversion is a scenario
map from allocations to the beliefs that would rationalize them within
the model; it is not evidence that beliefs explain observed differences
across firms.
...
which is low by construction throughout: the Anthropic-like and
OpenAI-like archetypes are effectively all-equity funded
(\(\ell = 0.05\)), the xAI-like archetype carries \(\ell = 0.15\) on its
secured debt, and even the hyperscaler carries only modest debt
(\(\ell = 0.10\)). This low-leverage structure is meant to capture the
venture-backed financing of AI labs and the availability of equity
capital at high valuations; were these firms to mature and take on
traditional debt (revolving credit facilities, secured notes, data
center ABS), leverage ratios would rise toward the Leland framework's
domain.\footnote{The leverage ratios in Table~\ref{tbl-firms} are round
  assumptions informed by qualitative evidence rather than measured
  market-value debt-to-capital ratios, which none of the private
  archetypes publishes: Anthropic is effectively all-equity; OpenAI
  secured a \$4B revolving credit facility; Alphabet carries modest net
  debt; xAI arranged \$5B+ in secured notes and term loans
...
Anthropic-like archetype's annual CapEx. Rescaling \(c\) in this way is
a change of units: with \(\kappa = c^{1/(\gamma-1)}\), the optimal
policy transforms as \(K \to K/\kappa\) and
\(X \to X/\kappa^{1-\alpha}\) and values scale by \(\kappa\), so every
ratio and percentage reported in the paper is unchanged. The credit
spreads and default probabilities of Section~\ref{sec-valuation} and
Internet Appendix H are evaluated at a fixed \((X, K)\) and are
invariant only if that evaluation point is rescaled with the units;
changing \(c\) alone at fixed \((X = 0.10, K = 1)\) changes the debt
face value \(\ell c K^\gamma\) and with it the spread.
...
revenue is \$3.7B in 2024 and \$12.5B in 2025, recognized-revenue
figures reported in the press; the CFO's January 2026 post gives
annualized run rates of \$2B, \$6B, and \$20B+ for 2023--2025 and does
not itself report recognized revenue, so the two series are not
interchangeable (the run-rate series would raise the 2025 figure and
lower the CapEx/Revenue ratio below one). CapEx is \$12B of Azure
inference and training spend, an operating commitment to cloud capacity
rather than owned infrastructure. For the Google-like hyperscaler,
...
\(\gamma\) has a non-monotone effect on both the trigger and the optimal
capacity: \(K_H^*\) is \(0.0107\) at \(\gamma = 1.20\), \(0.0034\) at
\(1.35\), \(0.0067\) at \(1.50\), and \(0.035\) at \(2.00\), with the
trigger tracing a similar U-shape. With the cost scale \(c\) held fixed,
a higher exponent lowers the marginal cost \(c\gamma K^{\gamma-1}\) for
capacities below the normalized unit \(K = 1\), where the optimum lies,
once \(\gamma\) is large enough for the exponent effect to dominate the
factor \(\gamma\). Panel (d) shows that higher operating cost \(\delta\)
raises both the trigger and the optimal capacity. As
